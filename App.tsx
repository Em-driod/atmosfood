...
const App: React.FC = () => {
  ...
  const addToCart = (item: FoodItem, proteins?: ProteinOption[]) => {
    setCart(prev => {
      ...
      if (existing) {
        return prev.map(i => i.id === item.id ? { ...i, quantity: i.quantity + 1, selectedProteins: proteins || i.selectedProteins } : i);
      }
  ...
  const getCategoryIcon = (cat: FoodCategory) => {
    switch (cat) {
      case FoodCategory.GRAINS: return <Soup size={20} />;
          ...
          <div className="flex-1 space-y-12 reveal-up">
            ...
            <h2 className="text-6xl md:text-7xl font-serif font-black leading-[0.95] tracking-tighter text-amber-950">
              ...
              <span className="relative">
                ...
                <span className="absolute -bottom-4 left-0 w-full h-2 bg-amber-400/20 -rotate-2 -z-10" />
              </span>
              <br />
            ...
            <div className="flex flex-col sm:flex-row items-center gap-8 pt-6 justify-center lg:justify-start">
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-amber-600/0 via-amber-600/20 to-amber-600/0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
              </button>
